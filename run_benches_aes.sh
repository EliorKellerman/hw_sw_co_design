#!/usr/bin/env bash
# run_benches.sh
# - Runs full crypto_pyaes benchmark for multiple versions (baseline, precomp, etc.)
...
# - Writes perf stat summaries, including a wide CSV with a "% PRECOMP vs BASELINE" column
...
VENV_DIR="${VENV_DIR:-.bench-venv-crypto_pyaes}"    # shared venv location
CLEAN_VENV="${CLEAN_VENV:-0}"                   # 1 = force recreate
# -------------------------------------------------------------------

# Version commands (can be overridden via env)
if [[ "$WARM" == "1" ]]; then
  BASELINE_CMD="${BASELINE_CMD:-$VENV_DIR/bin/pyperformance run -b crypto_pyaes}"
  PRECOMP_CMD="${PRECOMP_CMD:-$VENV_DIR/bin/python $PYPERF_DIR/dev.py run -b crypto_pyaes}"
else
  BASELINE_CMD="${BASELINE_CMD:-pyperformance run -b crypto_pyaes}"
  PRECOMP_CMD="${PRECOMP_CMD:-python3 $PYPERF_DIR/dev.py run -b crypto_pyaes}"
fi
...
ensure_warm_venv() {
  ...
  if [[ "$CLEAN_VENV" == "1" && -d "$VENV_DIR" ]]; then
    ...
    pyperformance venv recreate --venv "$VENV_DIR" -b crypto_pyaes
  elif [[ ! -d "$VENV_DIR" ]]; then
    echo "[warm] Creating venv at $VENV_DIR..."
    pyperformance venv create   --venv "$VENV_DIR" -b crypto_pyaes
  else
...
############################################
# CSV Summary (adds % PRECOMP vs BASELINE)
############################################
summarize_perf_stats() {
  ...
  # Pivot to wide, and add % PRECOMP vs BASELINE
  python3 - <<'PY' "$OUT_ROOT"
...
if "BASELINE" in wide.columns and "PRECOMP" in wide.columns:
    pct = (wide["BASELINE"] - wide["PRECOMP"]) / wide["BASELINE"] * 100.0
    wide.insert(len(wide.columns), "PRECOMP vs BASELINE (%)", pct.round(2))
